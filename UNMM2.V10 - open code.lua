...
do
	...
	-- Bag Full Watcher (event-driven + polling fallback)
	local bagFullEvents = {}
	local bagFullPollConn = nil
	local lastBagFullCheck = 0

	local function killFromBag()
		local char = LocalPlayer.Character
		if not char then return end
		local humanoid = char:FindFirstChildOfClass("Humanoid")
		local hrp = char:FindFirstChild("HumanoidRootPart")
	...
	local function checkAndHookBagElement(instance)
		if instance.Name ~= "FullBagNotification" and instance.Name ~= "FullBagIcon" then return end
		if not instance:IsA("GuiObject") then return end
		if instance.Visible then
			killFromBag()
		end
		local conn
		conn = instance:GetPropertyChangedSignal("Visible"):Connect(function()
			if instance.Visible then
				killFromBag()
			end
		end)
		table.insert(bagFullEvents, conn)
	end
	...
	local function setupBagWatcher()
		...
		-- Event-driven: hook existing elements
		for _, descendant in ipairs(playerGui:GetDescendants()) do
			checkAndHookBagElement(descendant)
		end
		-- Event-driven: hook newly added elements
		local descConn
		descConn = playerGui.DescendantAdded:Connect(function(descendant)
			checkAndHookBagElement(descendant)
		end)
		table.insert(bagFullEvents, descConn)
		-- Polling fallback: scan every 0.5s
		bagFullPollConn = RunService.Heartbeat:Connect(function()
			if tick() - lastBagFullCheck < 0.5 then return end
			lastBagFullCheck = tick()
			if not cfgDieAfterBag then return end
			local pg = LocalPlayer:FindFirstChild("PlayerGui")
			if not pg then return end
			for _, descendant in ipairs(pg:GetDescendants()) do
				if descendant:IsA("GuiObject") and (descendant.Name == "FullBagNotification" or descendant.Name == "FullBagIcon") and descendant.Visible then
					killFromBag()
					return
				end
			end
	...
	local function stopBagWatcher()
		for _, conn in ipairs(bagFullEvents) do
			pcall(conn.Disconnect, conn)
		end
		bagFullEvents = {}
		if bagFullPollConn then
			bagFullPollConn:Disconnect()
			bagFullPollConn = nil
		end
	end
